...
class PSCstudy(DocPublisher):
    ...
    def galactic_maps(self):
        ...
        The diffuse model used by 4FGL is `{self.diffuse_files[0]}`. It includes the "bubbles" using its patch. Its extent is limited,
        as shown in the plots above. It tends to have small-angle structure. This fact, and the limited extent is emphasized by the extrapolation to 1 TeV:
        {fig1}

        On the other hand, the uw model shows.
        """
        fig1 = plt.figure(figsize=(8,4), num=1)
        self.gflux.ait_plot(1e6, fig=fig1 ,label='')
    ...
    def plots(self):
        ...
        def roi_map(num):
            ...
            ax.set_title('4FGL-DR2 ROI locations')
            fig.tight_layout()
            return fig
        ...
        def stripe(label='Polar', num=2, energy=1000):
            ...
            fig.suptitle(title);    fig.set_facecolor('white')
