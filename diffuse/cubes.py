...
class DiffuseFactorCube(DocPublisher):
    ...
    def galactic_diffuse_model(self):
        ...
        fig1 =plt.figure(figsize=(10,4), num=1)
        fig1.caption = f'Flux for the patchless ;Galactic diffuse test model "{self.test_model_name}" at 1 GeV'
        dm.ait_plot( 1000., label='1 GeV', fig =fig1 )
    ...
    def dm7_comparison(self):
        ...
        hpratio.ait_plot( E, label=label, fig=fig1, log=False)
    ...
    def apply_factors(self):
        ...
        Here I multiply the DM7 model by the factor cube, representing the result with the 
        DM7 set of 28 energy planes, but nside={nside}.        
        The code to do this illustrates the power of this interface. In the code shown, `a` and  `b` are the two
        ...
        The result is written to the local file `{outfile}`

        ```        
        outfile = 'gll_iem_v07x_hpx.fits'
        dmx.to_FITS(outfile)
        ```

        {lsfile}

        It was subsequently copied to SLAC at `/nfs/farm/g/glast/g/catalog/pointlike/fermi/diffuse`
        ...
        dmx = HPcube(scube, energies, unit) 
